<div class="modal" id="editModal" tabindex="-1" role="dialog" aria-labelledby="Project Category">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<form class="form-horizontal" action="{{ route('projects.updateSubcategories')}}" id="editForm">{{ csrf_field() }}
				<div class="modal-body">
					...
					<div class="edit-result">
						<div class="form-group">
							<label for="edit-category" class="control-label">Project category:</label>
							<select required name="category" id="edit-category" class="form-control">
								<option value="">-select Project Category--</option>@foreach($categories as $category)
								<option value="{{ $category->id }}">{{ $category->name }}</option>@endforeach</select> <span class="text-danger" id="ecat" style="color: red"></span>
						</div>
						<div class="form-group">
